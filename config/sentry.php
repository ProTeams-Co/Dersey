...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | Only `user_id` and `order_number` are allowed to reach Sentry. This strips
    | Paymob secrets/tokens/hmacs, passwords, customer addresses/phone numbers,
    | and session cookies from every event before it leaves the app. See the
    | security rules in CLAUDE.md — never remove or weaken this callback.
    |
    */
    'before_send' => function (Event $event): Event {
        $scrub = static function (array $data) use (&$scrub): array {
            foreach ($data as $key => $value) {
                if (is_string($key) && preg_match('/paymob|secret|hmac|token|key|password|address|phone/i', $key)) {
                    $data[$key] = '[Filtered]';
        ...
        if (! empty($request)) {
            ...
            if (isset($request['headers']) && is_array($request['headers'])) {
                unset($request['headers']['Cookie'], $request['headers']['cookie']);
            }

            $event->setRequest($request);
        }

        $event->setExtra($scrub($event->getExtra()));
