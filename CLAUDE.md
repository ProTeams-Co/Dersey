...
- **⚠️ `OrderStatus::Returned` عمدًا من غير أي استرجاع مخزون تلقائي** (Batch 3.4 قرار 2) — على عكس `Cancelled` اللي بترجّع المخزون تلقائيًا عبر `OrderService::restoreInventory()`، الانتقال لـ `Returned` **مفيهوش أي side effect خالص** جوه `transitionTo()`. الفرق مقصود، مش سهو: طلب اتلغى قبل الشحن، البضاعة أصلًا لسه في المخزن؛ طلب اترجع بعد الشحن، البضاعة ممكن تكون تالفة أو ضايعة أو ما وصلتش أصلًا — الأدمن لازم يراجعها فعليًا قبل ما يرجّعها للمخزون، مش النظام يفترض إنها سليمة ويرجّعها آليًا. شاشة الطلبات (3.4) بتوضّح ده للأدمن (تنبيه + لينك مباشر لشاشة المخزون) بدل ما ترجّع المخزون بنفسها. **ممنوع حد يضيف استرجاع تلقائي لـ `Returned` من غير قرار صريح جديد.**
...
- **`Product::publicationBlockers()` بيتنادى مرتين في صفحة تعديل المنتج** — مرة في `edit.blade.php` (`$blockers = $model->publicationBlockers();` لعرض تنبيه الشروط الناقصة) ومرة تانية مستقلة في الـ partial المتضمّن `_tab-basic.blade.php` (نفس الاستدعاء بالظبط). كل استدعاء بيولّد نفس مجموعة الاستعلامات (بما فيها فحص الشرط 5 والشرط 6 من Batch 3.2-C) — يعني 4 استعلامات مهدورة في كل تحميل صفحة. الحل: تُحسب مرة واحدة في `ProductsController::edit()` وتتمرّر للـ view كمتغيّر (زي `$variants`/`$images`)، بدل ما الاتنين يعيدوا حسابها بشكل مستقل. مكتشَف في Batch 3.2-C-fix، مش مُصلَّح عمدًا لأنه بيلمس `edit.blade.php` (بنية مشتركة).
- **`App\Events\VariantLowStock` بيتبعت من غير أي listener** — `InventoryService::dispatchLowStockIfNeeded()` بيطلق الحدث ده كل مرة `stock_quantity` بينزل لحد `low_stock_threshold` أو أقل (عبر `adjust()`/`commit()`)، بس مفيش أي listener مسجَّل بيسمعه حاليًا (اتفحص فعليًا، صفر نتائج). إشعارات نفاد المخزون (إيميل/إشعار للأدمن) — Phase 8.
- **لينك "مراجعة variants الطلب" في شاشة الطلبات بيفتح لينك لكل SKU على حدة** — `InventoryTable` مفيهاش فلتر بـ variant ids متعددة، فلو الطلب فيه أكتر من عنصر، الأدمن بيلاقي أكتر من لينك (واحد لكل SKU) بدل لينك واحد مجمّع يعرض كل variants الطلب مرة واحدة. Phase 8.

Phase 8 هي اللي المفروض تتعامل مع البنود الستة دي مع بعض.
